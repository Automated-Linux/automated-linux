...
| `docker.cpuset_cpus` | CPUs pinned to the container — keep this below Docker Desktop's own vCPU allocation (`docker info --format '{{.NCPU}}'`) to leave headroom and avoid OOM during parallel builds (`make -j`) |
| `docker.memory` | Container memory limit — keep this below Docker Desktop's own memory allocation (`docker info --format '{{.MemTotal}}'`), not your Mac's total RAM; raising it here does nothing until Docker Desktop → Settings → Resources is given more to hand out |
...
- **Every `make -j` in the pipeline uses `nproc * 2`, not plain `nproc`.** `nproc` reflects `docker.cpuset_cpus`, so this deliberately oversubscribes the container's CPUs — normal practice for compiles, since no single job stays 100% CPU-bound the whole time (I/O, linking, etc.), and it noticeably shortens wall-clock time for CPU-heavy packages (GCC, glibc, the kernel). The tradeoff is peak memory: with `docker.cpuset_cpus: "0-11"` (12 cores → `-j24`) and heavy parallel C++ compilation (GCC itself, systemd), memory pressure scales with how many jobs are active at once, not just core count. **Check your actual Docker Desktop allocation before relying on this** — `docker info --format '{{.NCPU}} CPUs / {{.MemTotal}} bytes'` — and keep `docker.cpuset_cpus`/`docker.memory` comfortably below it (not below your Mac's raw hardware specs, which Docker Desktop's own VM doesn't see); the OOM-kill failure mode already documented under [Prerequisites](#prerequisites) gets easier to hit with heavier oversubscription, not harder.
