...
Automated-Linux builds a full Linux system **from source** (LFS-style) entirely through Ansible playbooks: it spins up a disposable Docker build container, cross-compiles a toolchain (binutils, GCC, glibc), natively builds the full LFS "Chapter 8" userland (~60 packages) inside a chroot with **systemd as PID 1**, compiles a Linux kernel, and boots the result in QEMU — all with a handful of commands run from your Mac.
...
| `docker.cpuset_cpus` | CPUs pinned to the container — keep this below your Mac's core count to leave headroom and avoid OOM during parallel builds (`make -j`) |
...
|---|---|---|
| [`docker.yaml`](playbooks/docker.yaml) | Mac (localhost) | Creates/starts the build container (with `--init`, see [Gotchas](#gotchas)), registers it in Ansible's inventory, bootstraps Python + sudo inside it |
...
- **The build container is not `--privileged`.** It runs with an explicit, minimal capability set (`SYS_ADMIN`, `MKNOD`, `SYS_CHROOT`, `DAC_OVERRIDE`, `CHOWN`, `FOWNER`, `SETUID`, `SETGID`) plus a device-cgroup rule for loop devices (major `7`) — just enough for the loop-mounted disk images, the `chroot` phase, and Ansible's `become`/`become_user` privilege switches. If a step ever fails with an unexpected `Operation not permitted`/`Permission denied` that traces back to a missing kernel capability rather than a real bug, the pragmatic fallback is to set `privileged: true` again in [`docker.yaml`](playbooks/docker.yaml) (removing `capabilities`/`devices`/`device_cgroup_rules`) rather than guessing at further capabilities to add one at a time.
...
- **Never mount the root image in the container while QEMU is running against it.** Both write to the same `.img` file with independent caches, which corrupts the ext4 filesystem (bad block bitmap checksums). `qemu.yaml` guarantees this by **deleting the build container outright** before booting (a loop mount or even just a stray `docker exec -it` shell with its cwd inside the mount can hold it busy, so a soft `umount` isn't reliable) and repairing the filesystem with Homebrew's `e2fsck` — installed automatically, no container round-trip needed. `docker.yaml` recreates the container from scratch on the next run.
...
- **If you poke around inside the container manually as root** (e.g. `docker exec ... mount -o loop ...` followed by ad hoc commands), files you touch outside of Ansible's `become_user: automated` tasks end up owned by `root` instead of `automated`. The next `packages/toolchain.yaml` run will then fail with `Permission denied` trying to overwrite them. Fix with `docker exec automated-linux-build chown -R automated:automated /mnt/automated-linux/usr /mnt/automated-linux/tools` (only needed if you've manually poked around — a pipeline run on its own never hits this).
