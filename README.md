...
- **Re-running `packages/toolchain.yaml` after it already completed once fails with `Permission denied`.** Its last task (`Change ownership of toolchain directory`) intentionally `chown -R root:root`s `usr`/`lib`/`lib64`/`var`/`etc`/`bin`/`sbin`/`tools` — the LFS book's "Changing Ownership" step, correct for a normal top-to-bottom run since nothing after it should write there as the `automated` user. But it makes a *second* run of `packages/toolchain.yaml` on the same image non-idempotent: the very first package's `make install` (as `become_user: automated`) hits a directory it no longer owns. The same thing happens if you poke around inside the container manually as root (e.g. `docker exec ... mount -o loop ...` followed by ad hoc commands) — anything you touch outside of Ansible's `become_user: automated` tasks ends up owned by `root`. Fix either way with `docker exec automated-linux-build chown -R automated:automated /mnt/automated-linux/usr /mnt/automated-linux/lib /mnt/automated-linux/lib64 /mnt/automated-linux/var /mnt/automated-linux/etc /mnt/automated-linux/bin /mnt/automated-linux/sbin /mnt/automated-linux/tools` before re-running (a single top-to-bottom pipeline run on its own never hits this).
