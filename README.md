...
- **A stale loop device can survive an image file being deleted and recreated.** `losetup -j <path>` matches by backing-file path, not inode, so if `automated-linux-root.img` is deleted and rebuilt, a loop device from a previous run can still be attached to the old (now-deleted) inode under the same path and get reused, silently pointing `mount`/`mkfs` at garbage. `prepare.yaml` detaches any such stale loop device whenever the image is freshly (re)created, before the reuse-detection step runs.
...
- **`e2fsck` against the raw (character) disk device on macOS can fail with `Error writing file system info: Invalid argument`** (exit code 9) even when the filesystem itself is fine — `hdiutil`'s `CRawDiskImage` raw-I/O path has alignment restrictions `e2fsck`'s final write trips over. `qemu.yaml` runs `e2fsck` against the buffered block device (`/dev/diskNs2`) instead of the raw one (`/dev/rdiskNs2`) to avoid this; slower, but this repair only runs once per boot test.
