...
def handle_res_output(res, color, event_type):
    if res is not None:
        ...
        if res.get('msg'):
            print(f'{color} - Download url: {res.get("url")}{ f" - {res.get('msg')}" if event_type in ["runner_item_on_failed"] else ""}')

class AnsibleRunner(Runner):
    def __init__(self, *args, **kwargs):
        config = RunnerConfig(private_data_dir='playbooks', playbook='automated-linux.yaml', quiet=True)
        config.prepare()
